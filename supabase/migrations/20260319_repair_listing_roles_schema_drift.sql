...
alter table public.listing_roles
  add column if not exists role_key text,
  add column if not exists display_name text,
  add column if not exists points_per_listing integer,
...
alter table public.listing_roles
  ...
  alter column points_per_listing set not null,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'listing_roles_role_key_format'
      and conrelid = 'public.listing_roles'::regclass
  ) then
    alter table public.listing_roles
      add constraint listing_roles_role_key_format
      check (role_key ~ '^[a-z0-9_]+$');
  end if;
end;
$$;

create unique index if not exists idx_listing_roles_role_key
  on public.listing_roles (role_key);
